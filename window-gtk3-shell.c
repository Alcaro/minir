...
/*
static void menu_activate(GtkMenuItem *menuitem, gpointer user_data)
{
	struct menudata_gtk3 * data=(struct menudata_gtk3*)user_data;
	unsigned int state=0;
	if (data->type==menu_checkbox)
	{
		state=gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(data->gtkitem));
		data->this->menumap[data->itemid].state=state;
	}
	if (data->type==menu_radio)
	{
		if (!gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(data->gtkitem))) return;
		state=data->radiostate;
		data->this->menumap[data->itemid].state=state;
	}
	if (data->onactivate && data->this->menu_allow_signals)
	{
		data->onactivate(data->itemid, state, data->userdata);
	}
}

static struct menudata_gtk3 * menu_create_menudata(struct window_gtk3 * this)
{
	unsigned int id=this->menudata_count;
	if (!(id&(id-1)))
	{
		this->menudata=realloc(this->menudata, sizeof(struct menudata_gtk3*)*((id+1)<<1));
	}
	this->menudata_count++;
	this->menudata[id]=malloc(sizeof(struct menudata_gtk3));
...
static char * menu_transform_name(const char * name, char accel)
{
	...
	char * ret=malloc(len+2);//NUL, extra _
	char * at=ret;
	for (int i=0;name[i];i++)
	{
		if (name[i]=='_') *(at++)='_';
...
static void menu_compile(struct window_gtk3 * this, GtkMenuShell* target, const struct menuitem * menu)
{
	GSList* radiogroup=NULL;
	unsigned int radioid=0;
	unsigned int radiogroupsize=0;
	while (true)
	{
		switch (menu->type)
		{
			case menu_end: return;
			case menu_skip: break;
			case menu_item:
			case menu_checkbox:
			case menu_radio:
				{
					char * label=menu_transform_name(menu->text, menu->key);
					
					GtkMenuItem* item=item;
					if (menu->type==menu_item) item=GTK_MENU_ITEM(gtk_menu_item_new_with_mnemonic(label));
					if (menu->type==menu_checkbox) item=GTK_MENU_ITEM(gtk_check_menu_item_new_with_mnemonic(label));
					if (menu->type==menu_radio)
					{
						if (radioid!=menu->id) radiogroup=NULL;
						GtkRadioMenuItem* radioitem=GTK_RADIO_MENU_ITEM(gtk_radio_menu_item_new_with_mnemonic(radiogroup, label));
						item=GTK_MENU_ITEM(radioitem);
						if (!radiogroup)
						{
							gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(radioitem), true);
							radiogroupsize=0;
						}
						radiogroup=gtk_radio_menu_item_get_group(radioitem);
						radioid=menu->id;
					}
					free(label);
					
					if (menu->type==menu_checkbox || (menu->type==menu_radio && !radiogroupsize))
					{
						if (menu->id >= this->menumap_count)
						{
							unsigned int oldsize=this->menumap_count;
							while (menu->id >= this->menumap_count) this->menumap_count*=2;
							unsigned int newsize=this->menumap_count;
							this->menumap=realloc(this->menumap, sizeof(struct menumap_gtk3)*this->menumap_count);
							for (unsigned int i=oldsize;i<newsize;i++)
							{
								this->menumap[i].menudataid=0;
								this->menumap[i].state=0;
							}
						}
						this->menumap[menu->id].menudataid=this->menudata_count;
					}
					
					if (menu->type==menu_radio && this->menumap[menu->id].state==radiogroupsize)
					{
						gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(item), true);
					}
					
					if (menu->type==menu_checkbox && this->menumap[menu->id].state!=0)
					{
						gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(item), true);
					}
					
					struct menudata_gtk3 * menudata=menu_create_menudata(this);
					menudata->this=this;
					menudata->gtkitem=item;
					menudata->type=menu->type;
					menudata->itemid=menu->id;
					menudata->radiostate=radiogroupsize;//used only for radio - possibly uninitalized for non-radios
					menudata->onactivate=menu->action;
					menudata->userdata=menu->userdata;
					g_signal_connect(item, "activate", G_CALLBACK(menu_activate), menudata);
					
					gtk_widget_set_sensitive(GTK_WIDGET(item), !menu->disabled);
					gtk_menu_shell_append(target, GTK_WIDGET(item));
			...
			case menu_separator:
				{
					GtkSeparatorMenuItem* item=GTK_SEPARATOR_MENU_ITEM(gtk_separator_menu_item_new());
					gtk_menu_shell_append(target, GTK_WIDGET(item));
				}
				break;
			case menu_submenu:
				{
					GtkMenuItem* item=GTK_MENU_ITEM(gtk_menu_item_new_with_label(menu->text));
					GtkMenu* submenu=GTK_MENU(gtk_menu_new());
					gtk_menu_item_set_submenu(item, GTK_WIDGET(submenu));
					menu_compile(this, GTK_MENU_SHELL(submenu), menu->submenu);
					gtk_widget_set_sensitive(GTK_WIDGET(item), !menu->disabled);
					gtk_menu_shell_append(target, GTK_WIDGET(item));
				}
				break;
			case menu_submenu_inline:
				menu_compile(this, target, menu->submenu);
				break;
		}
		menu++;
	}
...
static void menu_delete(struct window_gtk3 * this)
{
	if (this->menu)
	{
		for (int i=0;i<this->menudata_count;i++)
		{
			free(this->menudata[i]);
		}
		free(this->menudata);
	}
}
...
static void menu_create(struct window * this_, const struct menuitem * menu)
{
	...
	GtkMenuBar* newmenu=GTK_MENU_BAR(gtk_menu_bar_new());
	this->menu_allow_signals=false;
	
	this->menudata=NULL;
	this->menudata_count=0;
	if (!this->menumap)
	{
		this->menumap=malloc(sizeof(struct menumap_gtk3));
		this->menumap_count=1;
		this->menumap[0].menudataid=0;
		this->menumap[0].state=0;
	}
	
	menu_compile(this, GTK_MENU_SHELL(newmenu), menu);
	
	if (this->menu) gtk_widget_destroy(GTK_WIDGET(this->menu));
	this->menu=newmenu;
	gtk_grid_attach(this->grid, GTK_WIDGET(this->menu), 0,0, 1,1);
	gtk_widget_show_all(GTK_WIDGET(this->menu));
	
	this->menu_allow_signals=true;
}

static unsigned int menu_get_state(struct window * this_, unsigned int id)
{
	struct window_gtk3 * this=(struct window_gtk3*)this_;
	return this->menumap[id].state;
}

static void menu_set_state(struct window * this_, unsigned int id, unsigned int state)
{
	struct window_gtk3 * this=(struct window_gtk3*)this_;
	unsigned int dataid=this->menumap[id].menudataid;
	this->menu_allow_signals=false;
	if (this->menudata[dataid]->type==menu_checkbox)
	{
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(this->menudata[dataid]->gtkitem), state);
	}
	else
	{
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(this->menudata[dataid+state]->gtkitem), true);
	}
	this->menu_allow_signals=true;
}
*/
